    ...
    <div class="py-8 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex gap-1 mb-5 border-b border-gray-200">
            @foreach([
                ...
                <a href="{{ route('admin.users.index', ['tab' => $t['key'], 'q' => $search ?: null]) }}"
        ...
        <form method="GET" action="{{ route('admin.users.index') }}" class="flex gap-2 max-w-sm mb-5">
            ...
            <button class="px-4 py-2 rounded-lg text-white text-sm font-semibold shadow-sm shrink-0" style="background-color:#1a6b3c;">Search</button>
            @if($search !== '')
                <a href="{{ route('admin.users.index', ['tab' => $tab]) }}" class="px-3 py-2 rounded-lg text-sm font-medium text-gray-500 border border-gray-300 hover:bg-gray-50 shrink-0">Clear</a>
            @endif
        </form>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @empty
                        <tr><td colspan="7" class="px-5 py-14 text-center text-gray-400">
                            @if($search !== '')
                                No users match "<span class="text-gray-600 font-medium">{{ $search }}</span>" in this group.
                            @else
