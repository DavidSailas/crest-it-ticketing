            ...
            <table class="w-full text-sm border-collapse">
                <thead>
                    <tr class="bg-gray-50 border-b-2 border-gray-200">
                        ...
                        <th class="px-4 py-3 text-left font-semibold text-gray-600 border-r border-gray-200">Tickets</th>
                    ...
                    @foreach($departments as $dept)
                        <tr class="border-b border-gray-100 last:border-b-0" x-data="{ editing: false }">
                            <td class="px-4 py-3 border-r border-gray-100">
                                <div x-show="!editing" class="font-medium text-gray-800" x-text="'{{ $dept->name }}'"></div>
                                <form x-show="editing" method="POST" action="{{ route('admin.departments.update', $dept) }}" class="flex gap-2" id="dept-form-{{ $dept->id }}">
                                    @csrf @method('PUT')
                                    <input type="text" name="name" value="{{ $dept->name }}" class="rounded-md border-gray-300 text-sm py-1 focus:border-green-700 focus:ring-green-700" required>
                                </form>
                            </td>
                            <td class="px-4 py-3 border-r border-gray-100">
                            ...
                            <td class="px-4 py-3 text-gray-500 border-r border-gray-100">{{ $dept->tickets_count }}</td>
