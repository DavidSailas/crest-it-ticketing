    ...
    <div class="px-4 sm:px-6 py-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-5 mt-6 pt-6 border-t border-gray-100 text-sm">
            ...
            <div>
                ...
                @if($ticket->assistants->isNotEmpty())
                    <p class="text-xs text-gray-500 mt-1">
                        <span class="text-purple-600 font-medium">Assisting:</span>
                        {{ $ticket->assistants->pluck('name')->join(', ') }}
                    </p>
                @endif
            </div>
            <div>
        ...
        @else
            <div class="px-4 sm:px-6 py-4 bg-gray-50 border-t border-gray-100" x-data="{ status: '{{ old('status', $ticket->status) }}' }">
                <div class="flex flex-wrap gap-3 items-center">
                    ...
                    @elseif($ticket->isAssistedBy(auth()->user()))
                        <form method="POST" action="{{ route('tickets.unassist', $ticket) }}" class="sm:ml-auto">
                            @csrf
                            @method('DELETE')
                            <button class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg border border-purple-200 bg-purple-50 text-purple-700 text-sm font-medium hover:bg-purple-100">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                                Stop assisting
                            </button>
                        </form>
                    @elseif($ticket->canBeAssistedBy(auth()->user()))
                        <form method="POST" action="{{ route('tickets.assist', $ticket) }}" class="sm:ml-auto">
                            @csrf
                            <button class="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-white text-sm font-semibold shadow-sm hover:opacity-90" style="background-color:#6d28d9;">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" /></svg>
                                Assist on this ticket
                            </button>
                        </form>
                    @endif
                </div>
